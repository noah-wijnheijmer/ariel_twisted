...
class RobotBrain(nn.Module):
    ...
    def forward_control(self,
        ...
        ctrl_tensor = self.network(state_tensor).detach().numpy() * np.pi / 2
        # Avoid too nan, huge or inf warning
        if np.isnan(ctrl_tensor).any() or np.isinf(ctrl_tensor).any():
    ...
    def get_weights_as_vector(self) -> np.ndarray[Any, Any]:
        """
    ...
    def set_weights_from_vector(self, weight_vector: np.ndarray[Any, Any]) -> None:
        """
