...
from simulation.environments._amphitheatre import AmphitheatreTerrainWorld
from utils.tracker import Tracker
from utils.renderers import tracking_video_renderer
...
def visualize_champ(robot: CoreModule, individual: Individual, correct_for_bounding: bool, spawn_z: float, spawn_xy: list[float], path_settings: list[Any], brain_type: str, mode: str = "video", video_name: str | None = None) -> list[list[float]]:
    ...
    world.spawn(robot.spec, position=(spawn_xy[0], spawn_xy[1], spawn_z), correct_collision_with_floor=correct_for_bounding)
    ...
    if mode == "simple_runner":
        ...
        step_and_collect(steps)
        tracking_video_renderer(model, data, duration=sim_duration, video_recorder=video_recorder, geom_to_track="robot1_core", tracking_distance=6.0)
    elif mode == "launcher":
