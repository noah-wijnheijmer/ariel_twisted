...
EVAL_CONFIG = {"spawn_pos": [0, 0, 0.39], "target_pos": [0, 5, 0.5], "brain_type": "sf_cpg"}
...
def run_evolution_experiment(
    ...
    evaluate_population(mixed_twisty_population, EVAL_CONFIG["spawn_pos"], EVAL_CONFIG["target_pos"], EVAL_CONFIG["brain_type"])
    ...
    for generation in range(generations):
        ...
        for _ in range(twisty_maxspawn):
            chance = RNG.random()
            if chance <= twisty_p:
...
def main() -> None:
    ...
    visualize_champ(champion_robot, champion, EVAL_CONFIG["spawn_pos"], DATA_SETTINGS, EVAL_CONFIG["brain_type"], mode="launcher")
