...
def gecko_untwisted() -> CoreModule:
    """Gecko robot body.
    ...
    each rotated 90 degrees to each other.
    ...
    neck.sites[str(ModuleFaces.FRONT)].attach_body(
        body=abdomen.body,
        prefix="abdomen",
    )
    abdomen.sites[str(ModuleFaces.FRONT)].attach_body(
        body=spine.body,
        prefix="spine",
    )
    spine.sites[str(ModuleFaces.FRONT)].attach_body(
        body=butt.body,
        prefix="butt",
    )
    core.sites[str(ModuleFaces.LEFT)].attach_body(
        body=fl_leg.body,
        prefix="fl_leg",
    )

    fl_leg.sites[str(ModuleFaces.FRONT)].attach_body(
        body=fl_leg2.body,
        prefix="fl_flipper",
    )
    fl_leg2.sites[str(ModuleFaces.FRONT)].attach_body(
        body=fl_flipper.body,
        prefix="fl_flipper2",
    )

    core.sites[str(ModuleFaces.RIGHT)].attach_body(
        body=fr_leg.body,
        prefix="fr_leg",
    )

    fr_leg.sites[str(ModuleFaces.FRONT)].attach_body(
        body=fr_leg2.body,
        prefix="fr_flipper",
    )
    fr_leg2.sites[str(ModuleFaces.FRONT)].attach_body(
        body=fr_flipper.body,
        prefix="fr_flipper2",
    )

    butt.sites[str(ModuleFaces.LEFT)].attach_body(
        body=bl_leg.body,
        prefix="bl_leg",
    )
    bl_leg.sites[str(ModuleFaces.FRONT)].attach_body(
        body=bl_flipper.body,
        prefix="bl_flipper",
    )
    butt.sites[str(ModuleFaces.RIGHT)].attach_body(
        body=br_leg.body,
        prefix="br_leg",
    )
    br_leg.sites[str(ModuleFaces.FRONT)].attach_body(
