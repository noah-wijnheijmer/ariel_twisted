...
class NeuroEvolution:
    ...
    def tournament_parent_selection(self, population: Any) -> Population:
            ...
            if not ind.tags["mut"] and random.random() < self.config.mutation_rate:
                ...
                for i in range(len(ind.genotype)):
                    if random.random() < self.config.mutation_magnitude:
                        ind.genotype[i] += float(np.random.normal(loc=0.0, scale=self.config.mutation_scale)) # type: ignore
                        mutated = True
                if mutated:
                    ind.requires_eval = True
                    ind.tags["mut"] = True
    ...
    def evaluate(self,
        ...
        for ind in brain_pop:
            if ind.requires_eval:
                ...
                ind.fitness = self.fitness_function([0,0,0], history)
    ...
    def run(
        ...
        world.spawn(robot.spec, spawn_z=spawn_z, spawn_xy=self.config.spawn_xy, correct_for_bounding_box=self.config.cbb)
    ...
    def evolve(self, original_ind: Individual, use_gecko: bool=False, verbose: bool =False) -> JSONIterable:
        ...
        mujoco.set_mjcb_control(None)
        world = SimpleFlatWorld()
        world.spawn(robot.spec, spawn_z=self.config.spawn_z, spawn_xy=self.config.spawn_xy, correct_for_bounding_box=self.config.cbb)
        ...
        self.run(original_ind, best_init, nn_input_size, nn_output_size, mode="simple_runner", use_gecko=use_gecko) # save initial best brain video
        ...
        while num_evaluations < self.config.max_evals:
            ...
            if num_evaluations % (self.config.max_evals / 5) > (num_evaluations + current_evals) % (self.config.max_evals / 5): # 5 checkpoints
                self.run(original_ind, best_brain_in_gen, nn_input_size, nn_output_size, mode="launcher", use_gecko=use_gecko)
